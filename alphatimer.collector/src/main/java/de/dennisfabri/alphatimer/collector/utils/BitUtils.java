...
public class BitUtils {
    ...
    public BitSet extractUsedLanes(byte first, byte second) throws InvalidDataException {
        ...
        BitSet usedLanes = new BitSet();
        for (byte pos = 0; pos < 5; pos++) {
            usedLanes.set(4 - pos, isBitSet(first, pos));
        }
        for (byte pos = 0; pos < 5; pos++) {
            usedLanes.set(9 - pos, isBitSet(second, pos));
        }
        return usedLanes;
    }
