...
import stylesCss from 'themes/livetiming/styles.css';
...
window.Vaadin['_vaadintheme_livetiming_globalCss'] = window.Vaadin['_vaadintheme_livetiming_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin['_vaadintheme_livetiming_globalCss'].length === 0) || (!window.Vaadin['_vaadintheme_livetiming_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin['_vaadintheme_livetiming_globalCss'].push(target);
  }
  if (!document['_vaadintheme_livetiming_componentCss']) {
    
    document['_vaadintheme_livetiming_componentCss'] = true;
  }
