...
select max(lane1)                      as lane1,
       ...
       CONCAT(max(lane1), max(lane2), max(lane3), max(lane4), max(lane5), max(lane6), max(lane7), max(lane8),
              max(lane9), max(lane10)) as used_lanes_calculated,
       event,
       heat
from (select event,
             heat,
      ...
      from ares_message
      where message_type not in ('ReadyToStart', 'OfficialEnd', 'PreviousRaceResults', 'UnknownValue7')
        and kind_of_time not in ('Start')
     ) x
group by event, heat;
GO
