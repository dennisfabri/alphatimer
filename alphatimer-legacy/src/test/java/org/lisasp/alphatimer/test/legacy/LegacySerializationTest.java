...
class LegacySerializationTest {
    ...
    private static final String heatAsString = "Heat(event=2, heat=3, id=1, lanes={0=0:, 1=1: 2:03,450 (RaceTimes), 2=2: 3:54,560 (ResultsOfThePreviousRace), 3=3: 5:45,670 (ResultsOfTheRace), 4=4: 7:36,000 (BackupOfThePreviousRace), 5=5: 9:27,800 (RunningTime)})";
    ...
    @Test
    void emptyToXML() {
        String xml = LegacySerialization.toXML(new Heat[0]);

        assertEquals(emptyXmlRepresentation, xml);
    }

    @Test
    void toXML() {
        String xml = LegacySerialization.toXML(new Heat[] {heat});

        assertEquals(xmlRepresentation, xml);
    }

    private static final String emptyXmlRepresentation =
            "<AlphaServer.Heat-array/>";
